...
def iterate_register(mat, input_bit, crnt_state):
    eval1 = mat[0].substitute(x0=input_bit[0], x1=crnt_state[0][0], x2=crnt_state[0][1])
    eval2 = mat[1].substitute(x0=input_bit[1], x1=crnt_state[1][0])
    output = eval1 + eval2
    #output = [sum(eval.column(i)) for i in range(eval.ncols())]
    next_state = [[input_bit[i]] + crnt_state[i][:len(crnt_state[i])-1] for i in range(mat.nrows())]
    return list(output), next_state


def int_to_bit(int_len):
    output = []
    for i in range(2**int_len):
        oi = "{0:b}".format(i)
        oi = "0" * (int_len-len(oi)) + oi
        output.append([int(x) for x in oi])
    return output
...
def state_diagram_fun(input_mat, state_len):
    sd = {}             # sd == state_diagram
    ss = []             # ss == state_space
    for i in range(2**input_mat.ncols()):
        state = "{0:b}".format(i)
        state = "0" * (input_mat.ncols()-len(state)) + state
        breakpoint()
        for input in int_to_bit(state_len):
            output, next_state = iterate_register(input_mat, input, [int(x) for x in state])
            sd.update({(state, input): (next_state, output)})
...
def path_to_inputs(vit_node, state_len, state_state):
    ...
    path = [path[i:i+state_len] for i in range(0, len(path), state_len)]
    output = []
...
def main_viterbi(matrix, r, state_len):
    ...
    breakpoint()
    state_diagram, state_space = state_diagram_fun(matrix, state_len)
    state_space.sort()
    ...
    codeword = path_to_inputs(decoded, ncols, state_state)
    return codeword
